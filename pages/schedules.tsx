...
const ComponentPage: NextPage<ComponentPageProps> = ({ schedules, token }) => {
  const [nextSchedules, setNextSchedules] = useState<Schedule[]>(
    schedules.filter(
      (s) => new Date(s.scheduleAt).getTime() > new Date().getTime(),
    ),
  );
      ...
      <Page title={'Agendamentos'} id="schedules">
        ...
        <ul>
          {historySchedules.length === 0 && (
            <li>Não há histórico de agendamentos.</li>
          )}
          {historySchedules.map((schedule, index) => (
            <ScheduleItem key={index} token={token} schedule={schedule} />
          ))}
        </ul>
        <Footer
          buttons={[
