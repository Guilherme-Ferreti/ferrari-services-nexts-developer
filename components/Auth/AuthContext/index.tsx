...
const AuthProvider = ({ children }: AuthProviderProps) => {
  ...
  const onSubmitLogin = (data: FormDataLogin) => {
    axios
      .post<FormLoginResponse>(`/api/login`, data)
      .then(({ data: { token } }) => {
        setToken(token);
        redirectToNextURL();
      })
  ...
  const onSubmitRegister = (data: FormDataRegister) => {
    axios
      .post<FormLoginResponse>(`/api/register`, data)
      .then(({ data: { token } }) => {
        setToken(token);
        redirectToNextURL();
      })
  ...
  const onSubmitPasswordReset = useCallback(
    (data: FormDataPasswordReset) => {
      data.token = String(router.query.token) as string;

      axios
        .post<FormLoginResponse>(`/api/password-reset`, data)
        .then(({ data: { token } }) => {
          setToken(token);
          redirectToNextURL();
        })
  ...
  const getHashForm = useCallback(() => {
    ...
    if (
      ...
      (!email && ['login', 'forget'].includes(hash))
    ) {
      hash = 'email';
    }
